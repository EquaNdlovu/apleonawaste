...
?>
        ...
        $db = mysqli_connect('46.22.129.7', 'apl_waste_user', 'Upl5o73?', 'apleona_waste');
          $sql = "SELECT `waste_collector_key`, `waste_collector_name`, `waste_collector_country` FROM `waste_collector` WHERE `waste_collector_country` = '". $_SESSION['country'] ."' ORDER BY `waste_collector_key` DESC";
        $result = mysqli_query($db, $sql);
        $customerArray= array();
        while ($row = mysqli_fetch_array($result)) {
           array_push($customerArray, array('waste_collector_key'=>$row['waste_collector_key'],
                                            'waste_collector_name'=>$row['waste_collector_name'],
                                            'waste_collector_country'=>$row['waste_collector_country'] ));                                                    
        }

        $jsonArray = json_encode(array('data' => $customerArray), JSON_INVALID_UTF8_IGNORE);
       
          
     if (file_put_contents("assets/data/waste_collector.json", $jsonArray))
        ...
        <div class="modal fade" id="optionModal" tabindex="-1" role="dialog" aria-labelledby="optionModalLabel" aria-hidden="true">
          <div class="modal-dialog" role="document">
            <div class="modal-content">
              <div class="modal-header">
                <h5 class="modal-title" id="optionModalLabel">Add Waste Collector</h5>
              ...
              <div class="modal-body">
                <form action="<?php echo URLROOT; ?>/collectors/add" method="post">
                  <div class="form-group">
                    <label for="waste_collector_name">Name: <sup>*</sup></label>
                    <input type="text" name="waste_collector_name" class="form-control form-control-lg <?php echo (!empty($data['waste_collector_name_err'])) ? 'is-invalid' : ''; ?>">
                    <span class="invalid-feedback"><?php echo $data['waste_collector_name_err']; ?></span>
                  </div>
                  <div class="form-group" style="display:none">
                    <label for="waste_collector_country">Country: <sup>*</sup></label>
                    <input type="text" name="waste_collector_country" class="form-control form-control-lg <?php echo (!empty($data['waste_collector_country_err'])) ? 'is-invalid' : ''; ?>" value="<?php echo $_SESSION['country']; ?>">
          ...
          <div class="modal-dialog" role="document">
            <div class="modal-content">
              ...
              <div class="modal-body">
                <form action="<?php echo URLROOT; ?>/collectors/update" method="post">
                  <div class="form-group" style="display:none">
                    <label for="waste_collector_key">Key: <sup>*</sup></label>
                    <input type="text" name="waste_collector_key" id="waste_collector_key" class="form-control form-control-lg">
                  </div>
                  <div class="form-group">
                    <label for="waste_collector_name">Collector Name: <sup>*</sup></label>
                    <input type="text" name="waste_collector_name" id="waste_collector_name" class="form-control form-control-lg <?php echo (!empty($data['waste_collector_name_err'])) ? 'is-invalid' : ''; ?>">
                    <span class="invalid-feedback"><?php echo $data['waste_collector_name_err']; ?></span>
                  </div>
                  <div class="form-group" style="display:none">
                    <label for="waste_collector_country">Country: <sup>*</sup></label>
                    <input type="text" name="waste_collector_country" id="waste_collector_country" class="form-control form-control-lg <?php echo (!empty($data['waste_collector_country_err'])) ? 'is-invalid' : ''; ?>">
                    <span class="invalid-feedback"><?php echo $data['waste_collector_country_err']; ?></span>
                  </div>
                  <div class="modal-footer">
                    <!-- <input type="submit" class="btn btn-primary" value="Submit"> -->
                ...
                <!-- title -->
                <h1 class="page-title"> Waste Collectors </h1>
    ...
    <script src="<?php echo URLROOT; ?>/assets/javascript/pages/datatables-responsive-collectors.js"></script> 
    <!-- END PAGE LEVEL JS -->
    <!-- SweetAlert JS -->
    ...
    $(document).ready(function () {
      $('#dt-responsive').on('click', 'td.edit', function(e) {
        ...
        if (cl.indexOf("trash") >= 0) {
          ...
          $('#waste_collector_key').val(data[1]);
          $('#waste_collector_name').val(data[2]);
          $('#waste_collector_country').val(data[3]);
        }
      });
    });
  </script>
